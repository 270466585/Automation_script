...
class CaiGou(unittest.TestCase):
    ...
    def test_XSDD2(self):
        ...
        t.sleep(2)
        print(self.XS.assertion_test7())
        self.assertEqual(self.XS.assertion_test7(),'销售订单详情')
    ...
    def test_XSDD6(self):
        ...
        t.sleep(0.5)
        Data = self.XS.assertion_test6()
        print('销售订单总数为:{name}条'.format(name=Data[12:14]))
        ...
        try:
            if int(data[11])>0:
                ...
                print('模糊输入的订单编号为：{name},筛选结果订单编号为:{name1},所以,{name2}包含在{name3}中！'
                      .format(name=testData, name1=order,name2=testData, name3=order,))
            else:
